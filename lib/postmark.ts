...
export async function sendOrderConfirmationEmail(data: OrderEmailData) {
  ...
  console.log("API Token configured:", !!(POSTMARK_API_TOKEN && POSTMARK_API_TOKEN !== "your_postmark_api_token_here"))
  
  try {
    // Check if API token is configured
    if (!POSTMARK_API_TOKEN || POSTMARK_API_TOKEN === "your_postmark_api_token_here") {
      const errorMsg = "Postmark API Token is not configured correctly"
