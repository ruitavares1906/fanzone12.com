...
export const metadata = {
  title: "New Balance 574 | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de New Balance 574. Conforto e qualidade americana em cada passo.",
}

async function NB574Content({
  searchParams,
}: {
  ...
  // Filtrar produtos NB 574
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const nb574Products = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("new balance")
  )
  
  const total = nb574Products.length
  const inicio = (pagina - 1) * porPagina
  const produtos = nb574Products.slice(inicio, inicio + porPagina)
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos NB 574. 
...
export default function NB574Page({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>
}) {
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <NB574Content searchParams={searchParams} />
    </Suspense>
  )
}
