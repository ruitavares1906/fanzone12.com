...
export const metadata = {
  title: "New Balance 990 | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de New Balance 990. Conforto e qualidade americana em cada passo.",
}

async function NB990Content({
  searchParams,
}: {
  ...
  // Filtrar produtos NB 990
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const nb990Products = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("new balance")
  )
  
  const total = nb990Products.length
  const inicio = (pagina - 1) * porPagina
  const produtos = nb990Products.slice(inicio, inicio + porPagina)
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos NB 990. 
...
export default function NB990Page({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>
}) {
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <NB990Content searchParams={searchParams} />
    </Suspense>
  )
}
