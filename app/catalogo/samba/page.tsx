...
export const metadata = {
  title: "Adidas Originals Samba | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de Adidas Originals Samba. O clássico alemão que nunca sai de moda.",
}

async function SambaContent({
  searchParams,
}: {
  ...
  // Filtrar produtos Samba
  const allProducts = await getProdutos({ categoria: "sneakers" })
  let sambaProducts = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("adidas") &&
      product.categoria === "samba"
  )

  // Aplicar filtro de cor se especificado
  sambaProducts = aplicarFiltroCor(sambaProducts, cor)
  
  const total = sambaProducts.length
  const inicio = (pagina - 1) * porPagina
  const produtos = sambaProducts.slice(inicio, inicio + porPagina)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            O clássico alemão que nunca sai de moda
          </p>
        </div>

        {/* Filtros de Cor */}
        <SneakersColorFilter basePath="/catalogo/samba" />
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos Samba. 
...
export default function SambaPage({
  ...
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <SambaContent searchParams={searchParams} />
    </Suspense>
  )
}
