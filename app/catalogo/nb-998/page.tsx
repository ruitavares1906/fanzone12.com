...
export const metadata = {
  title: "New Balance 998 | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de New Balance 998. Conforto e qualidade americana em cada passo.",
}

async function NB998Content({
  searchParams,
}: {
  ...
  // Filtrar produtos NB 998
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const nb998Products = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("new balance") &&
      product.nome?.toLowerCase().includes("998")
  )
  
  const total = nb998Products.length
  const inicio = (pagina - 1) * porPagina
  const produtos = nb998Products.slice(inicio, inicio + porPagina)
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos NB 998. 
...
export default function NB998Page({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>
}) {
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <NB998Content searchParams={searchParams} />
    </Suspense>
  )
}
