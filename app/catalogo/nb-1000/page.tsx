...
export const metadata = {
  title: "New Balance 1000 | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de New Balance 1000. Conforto e qualidade americana em cada passo.",
}

async function NB1000Content({
  searchParams,
}: {
  ...
  // Filtrar produtos NB 1000
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const nb1000Products = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("new balance")
  )
  
  const total = nb1000Products.length
  const inicio = (pagina - 1) * porPagina
  const produtos = nb1000Products.slice(inicio, inicio + porPagina)
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos NB 1000. 
...
export default function NB1000Page({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>
}) {
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <NB1000Content searchParams={searchParams} />
    </Suspense>
  )
}
