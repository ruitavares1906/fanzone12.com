...
export const metadata = {
  title: "Adidas Originals Forum | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de Adidas Originals Forum. Design clássico e qualidade alemã.",
}
...
async function ForumContent({
  ...
  // Filtrar produtos Forum
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const forumProducts = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("adidas") &&
      product.categoria === "forum"
  )
  
  const total = forumProducts.length
  const inicio = (pagina - 1) * porPagina
  const produtos = forumProducts.slice(inicio, inicio + porPagina)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Design clássico e qualidade alemã
          </p>
        </div>
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos Forum. 
...
export default function ForumPage({
  ...
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <ForumContent searchParams={searchParams} />
    </Suspense>
  )
}
