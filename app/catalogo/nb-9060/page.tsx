...
export const metadata = {
  title: "New Balance 9060 | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de New Balance 9060. Conforto e qualidade americana em cada passo.",
}

async function NB9060Content({
  searchParams,
}: {
  ...
  // Filtrar produtos NB 9060
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const nb9060Products = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("new balance") &&
      product.nome?.toLowerCase().includes("9060")
  )
  
  const total = nb9060Products.length
  const inicio = (pagina - 1) * porPagina
  const produtos = nb9060Products.slice(inicio, inicio + porPagina)
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos NB 9060. 
...
export default function NB9060Page({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>
}) {
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <NB9060Content searchParams={searchParams} />
    </Suspense>
  )
}
