...
export const metadata = {
  title: "New Balance 2002R | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de New Balance 2002R. Conforto e qualidade americana em cada passo.",
}

async function NB2002RContent({
  searchParams,
}: {
  ...
  // Filtrar produtos NB 2002R
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const nb2002RProducts = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("new balance") &&
      product.nome?.toLowerCase().includes("2002r")
  )
  
  const total = nb2002RProducts.length
  const inicio = (pagina - 1) * porPagina
  const produtos = nb2002RProducts.slice(inicio, inicio + porPagina)
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos NB 2002R. 
...
export default function NB2002RPage({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>
}) {
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <NB2002RContent searchParams={searchParams} />
    </Suspense>
  )
}
