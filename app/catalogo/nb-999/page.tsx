...
export const metadata = {
  title: "New Balance 999 | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de New Balance 999. Conforto e qualidade americana em cada passo.",
}

async function NB999Content({
  searchParams,
}: {
  ...
  // Filtrar produtos NB 999
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const nb999Products = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("new balance") &&
      product.nome?.toLowerCase().includes("999")
  )
  
  const total = nb999Products.length
  const inicio = (pagina - 1) * porPagina
  const produtos = nb999Products.slice(inicio, inicio + porPagina)
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos NB 999. 
...
export default function NB999Page({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>
}) {
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <NB999Content searchParams={searchParams} />
    </Suspense>
  )
}
