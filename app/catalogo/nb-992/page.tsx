...
export const metadata = {
  title: "New Balance 992 | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de New Balance 992. Conforto e qualidade americana em cada passo.",
}

async function NB992Content({
  searchParams,
}: {
  ...
  // Filtrar produtos NB 992
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const nb992Products = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("new balance") &&
      product.nome?.toLowerCase().includes("992")
  )
  
  const total = nb992Products.length
  const inicio = (pagina - 1) * porPagina
  const produtos = nb992Products.slice(inicio, inicio + porPagina)
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos NB 992. 
...
export default function NB992Page({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>
}) {
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <NB992Content searchParams={searchParams} />
    </Suspense>
  )
}
