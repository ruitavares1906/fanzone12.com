...
export const metadata = {
  title: "New Balance 410 | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de New Balance 410. Conforto e qualidade americana em cada passo.",
}

async function NB410Content({
  searchParams,
}: {
  ...
  // Filtrar produtos NB 410
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const nb410Products = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("new balance") &&
      product.nome?.toLowerCase().includes("410")
  )
  
  const total = nb410Products.length
  const inicio = (pagina - 1) * porPagina
  const produtos = nb410Products.slice(inicio, inicio + porPagina)
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos NB 410. 
...
export default function NB410Page({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>
}) {
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <NB410Content searchParams={searchParams} />
    </Suspense>
  )
}
