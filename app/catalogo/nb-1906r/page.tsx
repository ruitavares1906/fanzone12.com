...
export const metadata = {
  title: "New Balance 1906R | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de New Balance 1906R. Conforto e qualidade americana em cada passo.",
}

async function NB1906RContent({
  searchParams,
}: {
  ...
  // Filtrar produtos NB 1906R
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const nb1906RProducts = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("new balance")
  )
  
  const total = nb1906RProducts.length
  const inicio = (pagina - 1) * porPagina
  const produtos = nb1906RProducts.slice(inicio, inicio + porPagina)
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos NB 1906R. 
...
export default function NB1906RPage({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>
}) {
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <NB1906RContent searchParams={searchParams} />
    </Suspense>
  )
}
