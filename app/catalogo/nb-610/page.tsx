...
export const metadata = {
  title: "New Balance 610 | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de New Balance 610. Conforto e qualidade americana em cada passo.",
}

async function NB610Content({
  searchParams,
}: {
  ...
  // Filtrar produtos NB 610
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const nb610Products = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("new balance") &&
      product.nome?.toLowerCase().includes("610")
  )
  
  const total = nb610Products.length
  const inicio = (pagina - 1) * porPagina
  const produtos = nb610Products.slice(inicio, inicio + porPagina)
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos NB 610. 
...
export default function NB610Page({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>
}) {
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <NB610Content searchParams={searchParams} />
    </Suspense>
  )
}
