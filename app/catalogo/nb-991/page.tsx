...
export const metadata = {
  title: "New Balance 991 | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de New Balance 991. Conforto e qualidade americana em cada passo.",
}

async function NB991Content({
  searchParams,
}: {
  ...
  // Filtrar produtos NB 991
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const nb991Products = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("new balance") &&
      product.nome?.toLowerCase().includes("991")
  )
  
  const total = nb991Products.length
  const inicio = (pagina - 1) * porPagina
  const produtos = nb991Products.slice(inicio, inicio + porPagina)
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos NB 991. 
...
export default function NB991Page({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>
}) {
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <NB991Content searchParams={searchParams} />
    </Suspense>
  )
}
