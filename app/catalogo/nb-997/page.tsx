...
export const metadata = {
  title: "New Balance 997 | fanzone12.pt",
  description: "Descobre a nossa coleção exclusiva de New Balance 997. Conforto e qualidade americana em cada passo.",
}

async function NB997Content({
  searchParams,
}: {
  ...
  // Filtrar produtos NB 997
  const allProducts = await getProdutos({ categoria: "sneakers" })
  const nb997Products = allProducts.filter(
    (product) => 
      product.subcategoria === "sneakers" && 
      product.marca?.toLowerCase().includes("new balance") &&
      product.nome?.toLowerCase().includes("997")
  )
  
  const total = nb997Products.length
  const inicio = (pagina - 1) * porPagina
  const produtos = nb997Products.slice(inicio, inicio + porPagina)
        ...
        {produtos.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Estamos a trabalhar para trazer mais modelos NB 997. 
...
export default function NB997Page({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>
}) {
  return (
    <Suspense fallback={<ProductsSkeleton />}>
      <NB997Content searchParams={searchParams} />
    </Suspense>
  )
}
